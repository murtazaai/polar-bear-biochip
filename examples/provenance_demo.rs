...
fn main() -> anyhow::Result<()> {
    ...
    println!("  Public key (65 B, uncompressed): {}…", &signer.public_key_hex()[..20]);
    ...
    let result = InferenceResult {
        ...
        raw_llm_response: r#"{"cognitive_state":"...","alert_level":"Normal","recommendations":[]}"#.to_string(),
    };
    ...
    println!("  EcdsaSigner::verify_signed  → {}", if valid { "✅ VALID" } else { "❌ INVALID" });
    ...
    println!("  EcdsaVerifier::verify       → {}", if ok2 { "✅ VALID" } else { "❌ INVALID" });
    ...
    println!(
        ...
        if tampered_valid { "❌ BUG: still valid!" } else { "✅ INVALID (tamper detected)" }
    );
    ...
    println!(
        ...
        if restored.public_key_hex() == signer.public_key_hex() { "✅ yes" } else { "❌ no" }
    );
